...
const KYCInformation = () => {
  ...
  const getCompanyCredential = async () => {
    let result = await Service.call('get', `/api/sc/kyc/company?admin_id=${dataSource.admin_id}`);
    setCompanyCredential(result);
  }

  const verifyCompanyCredential = async () => {
    const { company_kyc_id, admin_id, company_code, name, owner, description, industry, company_doc, company_size, address, found_date } = dataSource;
  ...
  const onChainProcess = async () => {
    const { company_kyc_id, admin_id, company_code, name, owner, description, industry, company_doc, company_size, address, found_date, is_company_doc_verified, status } = dataSource;
    if (_.isEmpty(dataSource)) return message.warning('cannot found kyc instance.');
    if (is_company_doc_verified <= 0) return message.warning('company document is not verified.');
    ...
    let resp = await Service.call('post', '/api/sc/kyc/company/credential/create', {
