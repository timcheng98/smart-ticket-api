...
const getTransactionByWallet = async (req, res) => {
  try {
    let result = await transactionModel.selectTransaction({ where: { sender: req.query.sender } });
    res.apiResponse({
      status: 1,
      result
    });
...
const getTransactionByUser = async (req, res) => {
  try {
    let result = await transactionModel.selectTransaction({ where: { user_id: req.query.user_id } });
    res.apiResponse({
      status: 1,
      result
    });
