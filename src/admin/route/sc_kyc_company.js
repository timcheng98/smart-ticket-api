...
module.exports = exports = {
  initRouter: (router) => {
    ...
    router.post('/api/sc/kyc/company/credential/create', createCompanyCredential);
...
const createCompanyCredential = async (req, res) => {
  try {
    ...
    let result = await kycModel.createCompanyCredential({admin_id}, id, hashHex);
    res.apiResponse({
      status: 1,
      result
    });
...
const renewCompanyCredential = async (req, res) => {
  try {
    const { admin_id, id, hashHex } = req.body;
    let result = await kycModel.renewCompanyCredential({admin_id}, id, hashHex);
    res.apiResponse({
      status: 1,
      result
    });
...
const burnCompanyCredential = async (req, res) => {
  try {
    const { admin_id, id, hashHex } = req.body;
    let result = await kycModel.burnCompanyCredential({admin_id}, id, hashHex);
    res.apiResponse({
      status: 1,
      result
    });
