...
const ERROR_CODE = {
  [-42001]: "media file type not accepted",
};
...
const postMedia = (req, res) => {
  try {
    ...
    if (!fileArr || fileArr.length <= 0) {
      throw new AppError(-42001);
    }

    const [file] = fileArr;
    const { filename, filepath, originalname } = file;
    const fileURL = url.resolve(
      getMediaConfig(scope).STATIC_SERVER_URL,
      filepath
    );
    res.apiResponse({
      originalname,
  ...
  return async (req, res, next) => {
    try {
      ...
      _.each(req.files, (fileArr) => {
        promises.push(saveMediaHandler({ admin_id, user_id, scope, fileArr }));
      });
