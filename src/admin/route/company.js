...
const postKyc = async (req, res) => {
  try {
    ...
    postObj.reference_no = `${moment().format("YYMMDD")}-${
      uuidv4().split("-")[1]
    }`;
...
const patchKyc = async (req, res) => {
  try {
    ...
    _.each(
      _.pick(req.body, ["status", "is_company_doc_verified"]),
      (val, key) => {
        postData[key] = _.toInteger(val);
      }
    );

    _.each(_.pick(req.body, ["found_date"]), (val, key) => {
...
const patchKycById = async (req, res) => {
  try {
    ...
    _.each(_.pick(req.body, ["reject_reason", "company_doc"]), (val, key) => {
      postData[key] = _.toString(val);
    });
